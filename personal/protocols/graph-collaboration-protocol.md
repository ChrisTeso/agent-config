...
## How Chris and Codex Execute Complex Work
...
The protocol changes the execution model, not the conversational relationship. Chris remains the goal owner and human gate. The primary Codex thread remains responsible for scope, dependency judgment, synthesis, and communication. Worker contexts perform bounded work. Fresh verifier contexts challenge consequential output. Tests and observed external state decide whether the graph passes.
...
### Chris: Goal Owner And Human Gate

Chris defines or approves:
...
### Primary Codex Thread: Orchestrator And Synthesizer
...
- Reporting status, blockers, gates, and decisions clearly to Chris.
...
## Toolbelt Messaging Example

A substantial Toolbelt Messaging integration can be executed as:

```text
Chris approves objective, scope, and human gates
                    |
                    v
       ...
       Foundation rebase and verification
        ...
        |                       |
        v                       v
 Stream workstream      Notification workstream
...
The important edges are foundation, shared identity and authorization contracts, worker and outbox semantics, and approved provider configuration. Stream implementation, notification implementation, provider-readiness inspection, and independent security review can fan out only after their shared contracts are stable.
...
Use the minimum graph that exposes the real dependencies. Preserve Chris's product control. Keep the primary conversation clear. Let bounded workers create breadth, fresh verifiers create skepticism, and external evidence decide what is actually done.
